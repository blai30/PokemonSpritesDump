...
builder.Services.AddSingleton<IImageConverter>(provider =>
{
    ...
    {
        ImageFormat.Png => new ImageConverterPng(provider.GetRequiredService<ILogger<ImageConverterPng>>()),
        ImageFormat.Jpg => new ImageConverterJpeg(provider.GetRequiredService<ILogger<ImageConverterJpeg>>()),
        ImageFormat.Webp => new ImageConverterWebp(provider.GetRequiredService<ILogger<ImageConverterWebp>>()),
        _ => throw new NotSupportedException($"Image format {imageOptions.Format} is not supported.")
    };
});
